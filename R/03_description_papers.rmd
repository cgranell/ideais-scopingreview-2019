...
```{r echo=FALSE}
ggplot(slice(non_stop_words_per_abstract, 1:30), aes(id, num_words, fill=year)) + 
  geom_col(show.legend = FALSE) +
  labs(x = NULL, y = "# de palabras", title="Top20 resúmenes por año y por # de palabras significativas") +
...
top_words %>%
  ...
  knitr::kable(caption = "Ranking detérminos más repetidos (si igual # repeticiones, ordenados por mayor # de resumenes)",
               format="html", escape = T, booktabs = TRUE) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
if (nrow(cloud_words) > 0) {  
  plot.new()
  p <- wordcloud(cloud_words$word, cloud_words$n,
            max.words = Inf,
            random.order = FALSE,
            fixed.asp = FALSE,
            rot.per = 0,
  ...
  #TODO: Save word cloudn into an image file
} else {
