...
papers %>%
  ...
  labs(#title="Temporal distribution of papers", 
    x="Year", 
    y="Number of papers") +
...
```{r top_journals, echo=FALSE}
# TODO: Finish table and recreate cloud words
papers %>%
  mutate(source = ifelse(!is.na(journal), journal, booktitle),
         type = ifelse(type=="InProceedings", "Conferences", "Journals")) %>%
...
non_stop_words_per_abstract %>%
  slice(1:10) %>%
  knitr::kable(caption = "Top10 abstracts ordered by number of significative words after removal stopwords.",
...
if (nrow(cloud_words) > 0) {  
  png(here("figs", "cloud_words_abstract.png"), width=1280,height=800)
...
if (nrow(cloud_keywords) > 0) {  
    png(here("figs", "cloud_keywords.png"))#, width=1280,height=800)
