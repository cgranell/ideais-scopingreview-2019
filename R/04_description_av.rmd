...
```{r parallel_set_plot, fig.height=12, fig.width=12}

gather_set_data(sankey, 1:3) %>% 
  ggplot(aes(x, id = id, split = y, value = freq)) +
  geom_parallel_sets(aes(fill = group), alpha = 0.3, axis.width = 0.2) +
  geom_parallel_sets_axes(axis.width = 0.2) +
  geom_parallel_sets_labels(colour = "white", angle = 360, size = 2) +
  theme_bw()
